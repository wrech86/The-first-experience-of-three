...
import vertexShader from '../../glsl/galaxy/vertex.glsl'
import fragmentShader from '../../glsl/galaxy/fragment.glsl'
const canvas = ref()
...
onMounted(()=>{
  ...
  const Galaxy = () =>{
    ...
    const ranD = new Float32Array(param.count)
    const randomXYZ = new Float32Array(param.count*3)
    ...
    for(let i=0;i<param.count;i++){
      ...
      position[i3] = Math.sin(angle)*radius
      position[i3+1] = 0
      position[i3+2] = Math.cos(angle)*radius
      //混乱程度
      ranD[i] = Math.random()
      //xyz随机度
      randomXYZ[i3] = randomX
      randomXYZ[i3+1] = randomY
      randomXYZ[i3+2] = randomZ
      //渐变
    ...
    geometry.setAttribute('randomXYZ',new THREE.BufferAttribute(randomXYZ,3))
